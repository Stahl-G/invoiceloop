...
- 8 类受控词表覆盖两个集上**全部**自由文本拼法,零个映不进。
- 类型声明的页面字面证据(词级 OCR,词序列匹配 + bbox 合并):
  88 份 81/86 = 94%,SEALED-2 91/99 = 92%。
- 被阻断的 13 份**全是真的误分类**,不是分词假象 —— 抽查四份:
...
- **产出即使后面全停也有价值**:它证明了抽取器的类型声明 8% 是错的。
...
- **2026-08-07 结论**(见 `docs/DOCTYPE_STAGE_D_2026-08-07.md`):
  主指标 29/56 = **51.8%** < 80% → **KILL**;旁证变体全 < 52%。
  主体方向不做机检,只走人工队列。不接线 gates。
...
若时间只够一条,先补强制项 —— 阶段 A 的产出(类型声明 8% 是错的)本身
已经是个可讲的发现,不必等整个体系做完。

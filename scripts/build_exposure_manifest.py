"""生成 docs/development_exposure_manifest.json(SEALED-1 排除池,高级裁决二)。
...
def main() -> None:
    ...
    for p in run_batch.sample(160):
        add(p.stem, "calibration-160(六轮实验设计/阈值全部见过它)",
    ...
    manifest = {
        "purpose": "SEALED-1 排除池:这些文档开发过程已暴露,不许进封箱评测",
    ...
    print("SEALED-1 取样必须排除以上全部;新增暴露(新 demo/新分析案例)"
          "必须先加进本清单再动手")

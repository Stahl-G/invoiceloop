...
> 该 `doc_id` 已注册证据片段的 OCR 文本中(规范化后 token 匹配 ≥80%)。
> 不匹配 → 拒绝该行,记 `draft_binding_rejected` 事件,**不进账本**。

拿第六轮实测数据验证这条规则的效力:

| 读图模型 | 作答 | 值在本文档 | 值在别的文档 | **会被拒绝** |
|---|---|---|---|---|
| Kimi K3 | 140 | 129 | 7 | 11 (7.9%) |
| Opus 5 | 146 | 140 | 2 | 6 (4.1%) |
| GPT 5.6 SOL | 168 | 57 | 106 | **111 (66%)** |
